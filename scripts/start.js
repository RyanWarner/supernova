...
const start = async () => {
  ...
  const ignore = ['src', 'scripts', 'config', './*.*', 'functions/build/client']
  let ignoreString = ''
  ignore.forEach(item => ignoreString = `${ignoreString} --ignore ${item}`)

  const script = nodemon(`${ignoreString} --inspect ${paths.serverBuild}/server.js`)
